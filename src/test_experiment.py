...
class CurrentSource(VirtualInstrument):
    ...
    def set_current(self, parsed_value):
        self.current = get_float_value(
            parsed_value, default=0, min_max=(-10, 10), units="A"
        )
...
class OhmExperiment(Experiment):
    ports = (PORT_V := 9001, PORT_I := 9002)

    def __init__(self, client_ip, resistance):
        super().__init__(client_ip)
        self.resistance = resistance
        self.instruments = {self.PORT_V: Voltmeter(), self.PORT_I: CurrentSource()}
...
class TestHooks(unittest.TestCase):
    ...
    def assertRun(self, port, command, *results):
        self.assertEqual(
            tuple(self.experiment.instruments[port].process(command)), results
        )
